...
class EnhancedSecurityService:
    ...
    # ------------------------------------------------------------------
    # On-ramp helper used by VelaFi integration
    # ------------------------------------------------------------------

    async def is_deposit_allowed(self, context: SecurityContext, amount: Decimal) -> bool:  # noqa: D401
        """Return **True** if the fiat deposit is considered safe to proceed.

        The helper evaluates velocity, geographic, and compliance thresholds
        re-using existing private checks in this service. It is lightweight and
        designed for *pre-transaction* gating – should run <10 ms in normal
        conditions.
        """

        # 1) Velocity / behavioural risk
        velocity_risk = await self._check_velocity_risk(context, amount)

        # 2) Geographic / IP reputation risk
        geo_risk = await self._check_geographic_risk(context)

        # 3) Compliance checks (KYC threshold, AML limits)
        compliance_flags = await self._check_compliance({"amount": amount}, context)

        high_risk = velocity_risk + geo_risk >= 1.0  # very rough aggregation
        has_compliance_issue = bool(compliance_flags)

        logger.debug(
            "Deposit risk-eval user=%s amount=%s velocity=%.2f geo=%.2f compliance=%s",
            context.user_id,
            amount,
            velocity_risk,
            geo_risk,
        ...
        return not high_risk and not has_compliance_issue
