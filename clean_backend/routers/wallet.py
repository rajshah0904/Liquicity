...
def calculate_fiat_balance(bridge_wallet: BridgeWallet) -> Decimal:
	"""Calculate total fiat balance from fiat_balance_by_rate buckets.
	
	Mapping format: { amount_key: rate_value } where amount_key is the fiat amount (str/num)
	and rate_value is the locked USDC/fiat rate. Sum the amount keys.
	"""
	mapping = bridge_wallet.fiat_balance_by_rate or {}
	if not mapping:
		return Decimal('0')
	
	total = Decimal('0')
	for amount_key, _rate in mapping.items():
		try:
