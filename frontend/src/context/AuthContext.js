...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (username, password, email, wallet_address, additionalData = {}) => {
    ...
    try {
      ...
      // Step 2: Update the wallet with user's preferred currency if provided
      if (userId && additionalData.currencyPreference) {
        try {
          await api.put(`/wallet/update/${userId}`, {
            currency: additionalData.currencyPreference,
            country_code: additionalData.countryCode || null
          });
        } catch (walletErr) {
          console.error('Error updating wallet currency:', walletErr);
          // Continue even if this fails - user is registered
        }
      }
      
      // Step 3: Store additional user information if available
      if (userId && Object.keys(additionalData).length > 0) {
        try {
          await api.post(`/user/${userId}/metadata`, {
            user_id: userId,
          ...
          console.error('Error storing user metadata:', metadataErr);
          // Continue even if this fails - user is registered
        }
      }
