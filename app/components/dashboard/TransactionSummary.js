...
const TransactionSummary = ({ totalTransactions, pendingTransactions, completedTransactions, failedTransactions, totalVolume, currency = 'USD' }) => {
      ...
      <ui_1.Card className="p-4">
        <div className="flex flex-col">
          ...
          <span className="text-2xl font-bold mt-1">
            {(0, utils_1.formatCurrency)(totalVolume, currency)}
          </span>
      ...
      <ui_1.Card className="p-4">
        <div className="flex flex-col">
          ...
          <div className="flex items-end gap-2">
            ...
            {pendingTransactions > 0 && (<span className="text-sm text-amber-500 mb-1">
      ...
      <ui_1.Card className="p-4">
        <div className="flex flex-col">
          ...
          <div className="flex items-end gap-2">
            ...
            {failedTransactions > 0 && (<span className="text-sm text-red-600 mb-1">
                ({failureRate}%)
              </span>)}
          </div>
          {failureRate > 10 && (<span className="text-xs text-red-500 mt-1">
              High failure rate - review transactions
            </span>)}
          {failureRate <= 10 && failureRate > 0 && (<span className="text-xs text-gray-400 mt-1">
              Within acceptable range
            </span>)}
          {failureRate === 0 && (<span className="text-xs text-gray-400 mt-1">
