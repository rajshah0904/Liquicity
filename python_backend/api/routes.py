...
from config.settings import ERROR_CODES, SUCCESS_CODES, settings
from core.bridge_api_client import BridgeAPIClient, BridgeError, BridgeTransferRequest
...
from core.usdc_payment_service import TransactionStatus, USDCError, USDCPaymentService
...
# Security middleware (allow missing tokens in DEBUG)
...
try:
    bridge_api_key = (
        settings.bridge_api_key.get_secret_value()  # type: ignore[attr-defined]
        if hasattr(settings, "bridge_api_key") and settings.bridge_api_key
        else "dev-bridge-key"
    )
    bridge_client = BridgeAPIClient(api_key=bridge_api_key)
except Exception:
    bridge_client = None
...
class WalletConnectRequest(BaseModel):
    ...
    wallet_address: Optional[str] = Field(None, description="User's wallet address (optional during pairing)")
...
class WalletConnectResponseWrapper(BaseModel):
    success: bool
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    ...
    # In development, allow missing/invalid tokens to speed up local testing
    if credentials is None:
        if settings.debug:
            return {"sub": "dev-user", "permissions": []}
        raise HTTPException(status_code=401, detail={"success": False, "error": {"code": "AUTHENTICATION_ERROR", "message": "Missing Authorization header"}})
    ...
    try:
        ...
        if settings.debug:
            # Fallback to a stub user in debug
            return {"sub": "dev-user", "permissions": []}
        raise HTTPException(
            status_code=401,
            detail={
...
# --- Dev utilities (enabled in DEBUG) ---
class DevTokenRequest(BaseModel):
    user_id: str
    permissions: Optional[List[str]] = []

@app.post("/api/v1/dev/token")
async def dev_issue_token(payload: DevTokenRequest):
    """Issue a short-lived JWT for local testing (DEBUG only)."""
    if not settings.debug:
        raise HTTPException(status_code=403, detail={"success": False, "error": {"code": "FORBIDDEN", "message": "Not available in production"}})
    try:
        token = authentication_service.generate_secure_token({"sub": payload.user_id, "permissions": payload.permissions or []}, expiration=3600)
        return {"success": True, "token": token}
    except Exception as e:
        # Always return JSON so curl prints something
...
@app.post("/api/v1/wallet/connect", response_model=WalletConnectResponseWrapper)
async def connect_wallet(
    ...
    try:
        # Allow empty wallet address during initial pairing
        ...
        # Generate QR code and URI (if Node microservice is used, service overrides URI)
        qr_code_url = await walletconnect_service.generate_qr_code(session.id)
        uri = walletconnect_service._create_walletconnect_uri(walletconnect_service.sessions[session.id])
        
        logger.info(f"Created WalletConnect session {session.id} for user {request.user_id}")
        
        payload = WalletConnectResponse(
        ...
        # Return in a shape wc-qr-test expects: { success, data }
        return WalletConnectResponseWrapper(success=True, data=payload)
