...
# --- Backward-compatible facades expected by api/routes.py ---

class SecurityValidator:
    """Lightweight validator facade used by routes.

    Provides the subset of validation helpers referenced by the API layer.
    """

    def validate_wallet_address(self, address: str, chain_type: str) -> bool:
        return security_service.validate_wallet_address(address, chain_type)
...
class RateLimiter:
    """Simple in-memory allow-all rate limiter stub for dev/testing."""

    async def allow(self, key: str, limit_type: str = "api") -> bool:
        return True


def calculate_risk_score(context: SecurityContext) -> float:
    """Very basic risk score used by the demo routes."""
    return 0.1


# Symbols imported by routes
authentication_service = security_service
authorization_service = security_service
security_validator = SecurityValidator()
rate_limiter = RateLimiter()
encryption_service = security_service
