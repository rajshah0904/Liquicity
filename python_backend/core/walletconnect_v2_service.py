...
import qrcode
import websockets
from config.settings import ERROR_CODES, settings
from core.security import SecurityException, encryption_service, security_validator
...
class WalletConnectV2Service:
    ...
    async def create_session(
        ...
        # Validate inputs – allow empty wallet_address during initial pairing
        if wallet_address:
        ...
        # WalletConnect topics are random 32-byte hex strings
        ...
        # If a WalletConnect microservice is configured, delegate pairing creation to it
        try:
            if getattr(settings, "walletconnect_ms_url", None):
                await self._delegate_pairing_creation(session)
            else:
                await self._start_websocket_connection(session_id)
        except WalletConnectError as e:
            logger.warning("WalletConnect setup warning during session creation: %s", e.message)
    ...
    async def generate_qr_code(self, session_id: str) -> str:
        ...
        # Use delegated URI if available; otherwise construct locally
        uri = session.pairing_uri or self._create_walletconnect_uri(session)
    ...
    def _create_walletconnect_uri(self, session: WalletConnectSession) -> str:
        """Create WalletConnect v2 pairing URI.

        Format: wc:{topic}@2?relay-protocol=irn&symKey={symKey}&projectId={projectId}
        """
        return (
    ...
    async def _delegate_pairing_creation(self, session: WalletConnectSession) -> None:
        """Delegate pairing to Node microservice using official SignClient.

        The microservice will create the pairing and emit an approval when the wallet accepts.
        """
        import httpx
        ms_base = settings.walletconnect_ms_url.rstrip('/')
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.post(f"{ms_base}/pairing/create", json={})
                resp.raise_for_status()
                data = resp.json().get("data", {})
                uri = data.get("uri")
                if uri:
                    # Override with SDK-generated URI (topic and symKey)
                    try:
                        session_uri_topic = uri.split(":")[1].split("@")[0]
                        # Extract symKey from query string
                        from urllib.parse import parse_qs, urlparse
                        parsed = urlparse(uri)
                        qs = parse_qs(parsed.query)
                        sym_key = (qs.get("symKey") or qs.get("symkey") or [None])[0]
                    except Exception:
                        session_uri_topic, sym_key = session.topic, None
                    session.topic = session_uri_topic
                    if sym_key:
                        session.sym_key = sym_key
                    session.pairing_uri = uri
                    # Store back
                    self.sessions[session.id] = session
        except Exception as e:
            raise WalletConnectError(
                error_code=ERROR_CODES["INTERNAL_ERROR"],
                message=f"Microservice pairing failed: {str(e)}"
            )
    ...
    async def _start_websocket_connection(self, session_id: str):
        ...
        try:
            # Connect to WalletConnect relay; append projectId as query param
            ...
            # Surface as WalletConnectError only if not in debug; otherwise non-fatal
