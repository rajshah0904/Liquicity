...
try:
    ...
    except ModuleNotFoundError:  # solana>=0.28 moved to solders
        from solders.system_program import TransferParams, transfer  # type: ignore
    from solana.rpc.commitment import Commitment
except ModuleNotFoundError:
    # Provide soft-fallbacks for local testing when solana extras are not present
    SolanaClient = None  # type: ignore
    SolanaTransaction = None  # type: ignore
    TransferParams = None  # type: ignore
    transfer = None  # type: ignore
    Commitment = None  # type: ignore
import base64
import uuid

from config.settings import ERROR_CODES, NETWORK_CONFIGS, settings
from core.security import SecurityException, security_validator
from core.walletconnect_v2_service import WalletConnectError, WalletConnectV2Service
